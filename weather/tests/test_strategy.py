...
class TestScoreBuckets(unittest.TestCase):
    ...
    def test_scores_sorted_by_ev(self):
        markets_data = _load_fixture("simmer_markets.json")
...
class TestStrategyIntegration(unittest.TestCase):
    """Full dry-run with mocked APIs."""

    def _make_mock_client(self) -> MagicMock:
        client = MagicMock(spec=SimmerClient)
        client.get_portfolio.return_value = {
            "balance_usdc": 50.0,
            "total_exposure": 10.0,
            "positions_count": 2,
        }
        client.fetch_weather_markets.return_value = _load_fixture("simmer_markets.json")["markets"]
        client.get_positions.return_value = _load_fixture("simmer_positions.json")["positions"]
        client.get_market_context.return_value = None  # No safeguards blocking
    ...
    def test_dry_run_no_errors(self, mock_noaa):
        ...
        client.fetch_weather_markets.assert_called_once()
    ...
    def test_no_trade_when_no_edge(self, mock_noaa):
        ...
        markets = _load_fixture("simmer_markets.json")["markets"]
        for m in markets:
            m["external_price_yes"] = 0.95

        client = self._make_mock_client()
        client.fetch_weather_markets.return_value = markets
