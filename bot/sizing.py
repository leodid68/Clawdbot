...
def kelly_fraction(p: float, market_price: float, fraction: float = 0.25) -> float:
    """Fractional Kelly criterion for prediction markets.

    Simplified formula: f* = (p - price) / (1 - price).
...
def position_size(
    ...
    """
    frac = kelly_fraction(probability, price, fraction=kelly_frac)
    if frac <= 0:
        return 0.0
    ...
    logger.debug(
        "Kelly sizing: p=%.3f price=%.3f frac=%.4f → $%.2f",
        probability, price, frac, size,
    )
    return round(size, 2)
...
def check_risk_limits(
    state: TradingState, config: Config, new_trade_usd: float,
) -> tuple[bool, str]:
